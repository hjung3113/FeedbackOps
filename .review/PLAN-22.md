...
| D-21 | `GET /vocs/:id` payload includes `attachments: LinkedAttachment[]` for the VOC body **and** `attachments: LinkedAttachment[]` on every `conversation_timeline[]` entry. Same `LinkedAttachment` shape on `GET /vocs/:id/conversation` items. `GET /vocs` list rows carry `attachment_count: number` (computed via bulk subquery on the page, no full JOIN). `LinkedAttachment` = `{ id, name, size_bytes, mime_type, uploaded_by_actor_id, created_at, linked_at }` — `storage_key`/`storage_uri` NOT exposed; clients reference by `id`. Archived rows excluded. | Hotfix `fix/22-detail-attachments-cursor-submit-warning` (2026-05-22) |
| D-22 | `GET /vocs/:id/conversation` `?cursor=` query parameter is **optional**. First-page call (no cursor) returns the oldest entries with a `next_cursor` if `has_more`. The FE infinite-query hook issues its first GET without a cursor by design; the prior "cursor required" contract surfaced a 422 on every detail-panel open. | Hotfix `fix/22-detail-attachments-cursor-submit-warning` (2026-05-22) |
| D-23 | VOC create form surfaces an inline alert above the submit action bar whenever any attachment row is in `error` state (per-file size cap, unsupported MIME, or upload failure). The submit button stays disabled while the alert is visible. Without the alert, submit-disabled looks broken (no user-facing reason). Source of truth for row state is `AttachmentDropzone`; the form subscribes via `onErrorCountChange`. | Hotfix `fix/22-detail-attachments-cursor-submit-warning` (2026-05-22) |

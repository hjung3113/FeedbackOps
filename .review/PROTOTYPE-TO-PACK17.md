...
| `.badge.badge-reporter` | `ReporterStatusBadge` | **Post-PR-59 pill (b05d7b7):** `inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold`. Background: `rgb(var(--status-reporter-{enum}) / 0.14)`. Foreground: `rgb(var(--status-reporter-{enum}) / 1)`. Leading 6px dot at foreground color. **Token note:** `--status-reporter-*` tokens in `tokens.css` are raw RGB triplets; must be wrapped in `rgb(…)` — bare `var(--token)` is not a valid CSS color. | Use `<ReporterStatusBadge status={voc.reporterStatus} />`. Component handles the wrapping. Never reference the token directly. |

...
function TasksScreen({ view, selectedParam, scope }) {
  // TaskMyView + TaskInboxView live in screen-tasks-views.jsx (Pack 19 split);
  // read them off window so the dispatcher doesn't depend on file load order.
  ...
  if (view === 'my')         return <window.TaskMyView scope={scope} />;
  if (view === 'inbox')      return <window.TaskInboxView scope={scope} />;
  return <TaskBoardView scope={scope} selectedParam={selectedParam} />;
}

Object.assign(window, { TasksScreen, TaskBoardView, TaskRequestView, TaskBacklogView, TaskBacklogRow, TaskDetailPanel });

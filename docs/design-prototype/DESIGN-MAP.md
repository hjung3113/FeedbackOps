...
| `integration-coverage` | `screen-coverage.jsx` | `/integration/coverage` | `routes/_authed/integration/coverage.tsx` | `features/integration/routes/CoverageRoute.tsx` in `PageShell`. Search: `managedSystem` uuid \| all. One-hop rows use the shared `DASHBOARD_HOP_ROUTES` map (`packages/shared/src/dashboard.ts`); the per-Managed-System table renders `by_managed_system` | `registered` | `api/dashboard.md` (`GET /dashboard/summary`) |

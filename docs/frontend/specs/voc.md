...
Backend sanitization is authoritative (ADR-0011): the editor enforces the toolbar allowlist client-side as UX guidance only. The server rejects nodes/marks outside the surface allowlist with `code: 'rich_content.disallowed_node'` (proposed — confirm in S3-001 / ADR-0012 enum addition).
...
| Q4 (AA owner vs MS default owner precedence) | When the actor creates a VOC, multiple default-owner rules may apply (per `docs/implementation/03-api-contracts.md:83-92`): MS default owner, AA owner team, capability rules. Order of precedence determines whether the Create response prefills owner. UI impact: the Triage screen's "Owner 없음" red tag wording assumes no default owner ever resolves; if AA defaulting is live in Slice 3, copy needs to differentiate "default-resolved-but-still-untriaged" from "unassigned". | Triage row meta; Triage panel Owner picker initial value | Backend (precedence rule lives in service code) | Before S3-002 (PATCH /vocs/:id behaviour) |

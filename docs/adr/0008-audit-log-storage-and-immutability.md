...
`event_type` values are drawn from the canonical `AUDIT_EVENT_TYPES` list in `packages/shared/src/enums/audit-events.ts`, used by the Zod enum `auditEventTypeSchema`; each event's `detail` schema is the separate registry `AUDIT_EVENT_DETAIL_SCHEMAS` in that same file. The naming convention is **snake_case verbs scoped by the audited concept** (e.g. `permission_requested`, `permission_approved`, `task_request_approved`, `reporter_facing_status_changed`). For audit events listed in `docs/implementation/05-permission-policy.md` (and any other domain policy doc that locks an event vocabulary), this ADR explicitly **defers to that doc's verb list verbatim** — the policy file is the source of truth. The earlier illustrative example of `subject_type.verb` (e.g. `permission_request.decided`) was a non-binding stylistic suggestion; it is superseded for events the policy doc names directly. Cross-domain queries still work because `subject_type` and `subject_id` remain explicit columns, independent of the `event_type` string shape.

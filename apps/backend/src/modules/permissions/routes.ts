...
export const permissionsRoutes: FastifyPluginAsync<
  PermissionsRoutesOptions
> = async (app, opts) => {
  const {
    sessionService,
    checkService,
  ...
  app.route({
    ...
    handler: async (req, reply) => {
      ...
      const decision: Decision = await checkService.checkCapability(
        ...
        capability,
        {
          ...
          ...(q.managed_system_id !== undefined
            ? { managed_system_id: q.managed_system_id }
            : {}),
        },
      );
      ...
      const openRequestSummary = await requestService.findOpenRequestSummary(
        actor,
        capability,
        {
          workspace_id: sess.workspace_id,
      ...
      const state: FrontendState = toFrontendState(
        decision,
        openRequestSummary,
      );
  ...
  app.route({
    ...
    handler: async (req, reply) => {
      ...
      const parsed = z
        .object({
          status: z
            .enum(["pending", "needs_more_info", "approved", "rejected", "all"])
            .optional(),
        })
        .safeParse(req.query);
      if (!parsed.success) {
  ...
  const decisionRoutes: Array<{
    ...
    },
    {
      ...
      invoke: (actor, id, body, idempotencyKey) =>
        decisionService.needMoreInfoRequest(
          actor,
          id,
          body as { note: string },
          { idempotencyKey },

...
//
// How far the backfill rescues a dropped enqueue — both halves are covered,
// which is what makes swallowing errors here defensible:
//
//  - **Create**: the VOC has no row at the active version, so
//    `selectVocsNeedingEmbedding` finds it on the next cron run.
//  - **Edit**: the VOC's row is older than the VOC itself
//    (`voc_embeddings.updated_at < vocs.updated_at`), so the same query finds
//    it. This half was originally uncovered and left a permanently stale
//    vector; the timestamp signal in `embedding/repo.ts` closes it.
//

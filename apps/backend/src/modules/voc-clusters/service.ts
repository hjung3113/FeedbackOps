import type {
  AddVocClusterMemberRequest,
  ApplyVocClusterPublicUpdateRequest,
  CreateVocClusterRequest,
  ListSameManagedSystemCandidatePeersResponse,
  ListVocClustersResponse,
  UpdateVocClusterRequest,
  VocClusterDto,
  VocClusterMemberDto,
  VocClusterPublicUpdateCandidateRequest,
  VocClusterPublicUpdateOutcome,
} from '@fops/shared';
...
async function canReadCluster(
  ...
  return isFindingInReadScope(
    await actorFindingReadScope(deps.db, actor, { requireElevatedRole: true }),
    managedSystemId,
  );
}
...
export function createVocClustersService(deps: VocClustersServiceDeps) {
  ...
  return {
    ...
    ...createVocClusterConversion(deps, canManageCluster),
  };
}

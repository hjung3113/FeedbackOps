...
async function captureWorkHandler(
  ...
  log: JobLog,
): Promise<(jobs: Array<{ data: TaskReleasedReviewCandidatesPayload }>) => Promise<void>> {
  let captured:
    | ((jobs: Array<{ data: TaskReleasedReviewCandidatesPayload }>) => Promise<void>)
    | undefined;
  const boss = {
    getQueues: vi.fn(async () => [{ name: TASK_RELEASED_REVIEW_CANDIDATES_QUEUE }]),
    work: vi.fn(
      async (_queue: string, options: unknown, handler: NonNullable<typeof captured>) => {
...
describe("released-review-candidates job logging (ADR-0013)", () => {
  ...
  it("rethrows service failures after a job.failure line (pg-boss retry contract)", async () => {
    ...
    await expect(handler([{ data: payload() }])).rejects.toBe(boom);

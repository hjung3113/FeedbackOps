...
- **Integration suites are gated on env, and the gate resets the database.** All 90 integration suites use `describe.skipIf(!runIntegration)` where `runIntegration` requires `DATABASE_URL`, `DATABASE_URL_MIGRATE`, and (in most files) `WORKSPACE_ID`. With no env exported — the default `pnpm --filter backend test` — they all skip and no database is touched. Export the env (`set -a && . ./.env && set +a`) and the vitest `globalSetup` at `src/test-support/global-setup.ts` **truncates every product table and re-seeds before the run**, so a run's result never depends on how many runs came before it (#205). Point `DATABASE_URL`/`DATABASE_URL_MIGRATE` at a throwaway database if the contents of your dev database matter to you; `TEST_DB_NO_RESET=1` opts out of the reset and back into non-reproducible counts. Deviating from the `skipIf` idiom is a bug: a bare `describe` runs its `beforeAll` in the default gate and fails the suite.

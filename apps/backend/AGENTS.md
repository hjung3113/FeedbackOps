...
- **`test:integration` is the gate; the default `test` includes DB-free unit suites and is not sufficient for DB-backed verification.** Integration suites use `describe.skipIf(!runIntegration)` where `runIntegration` requires `DATABASE_URL`, `DATABASE_URL_MIGRATE`, and (in most files) `WORKSPACE_ID`, so a bare `pnpm --filter backend test` skips every one of them and touches no database. `pnpm --filter backend test:integration` (`scripts/test-integration.sh`) exports the repo `.env` and runs the same suite with the integration path live; it forwards extra args to vitest, so a narrow filter still works. `FEEDBACKOPS_ENV_FILE=<path>` points it at a different env file. **DB-backed integration verification is complete only from the result of `pnpm --filter backend test:integration`** — `src/test-support/__tests__/integration-gate.test.ts` is an ungated suite and it fails the default run for exactly that reason (#204). On a machine with no Postgres, `ALLOW_SKIPPED_INTEGRATION=1` opts out explicitly; a run carrying that flag is not a gate result and must not be reported as one.
...
- **Never assert an exact set over `core.actors`.** The table is shared across the whole run and some suites create an actor on purpose without removing it (the rate-limit case in `modules/voc/__tests__/post-reporter-reply` needs a fresh actor per run to get a clean rate-limit bucket). A suite that asserts the exact list of non-system actors passes alone and fails in the full run, in an order-dependent way — that was #304. Assert over the **set difference** the suite itself causes (snapshot before, snapshot after, compare the delta by `external_id`), and put arity assertions *after* the set assertions so the failure message names the offending actor instead of reporting a count.

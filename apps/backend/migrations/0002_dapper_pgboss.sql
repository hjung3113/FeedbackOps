-- ADR-0009 + ADR-0015:88: pre-create the `pgboss` schema and grant fops_app
-- DML-only access. Choice (A) from the Slice 1 spec: keep the running app
-- DDL-less so it stays consistent with ADR-0008 (only fops_migrate may change
-- shape). pg-boss is started with `migrate: false, createSchema: false` so it
-- never tries to CREATE/ALTER anything from the fops_app connection — it just
-- runs DML against the tables and stored functions we install here.
--
-- The body below is the output of `pg-boss@12.18.2`'s
-- `getConstructionPlans('pgboss')`, with the script's own outer
-- BEGIN/COMMIT/SET LOCAL/pg_advisory_xact_lock stripped because drizzle's
-- migrator already wraps every migration in a transaction. If we upgrade
-- pg-boss to a release with a different schema version, the upgrade is a new
-- migration generated via `getMigrationPlans('pgboss', <fromVersion>)` —
-- never let pg-boss self-migrate from the running app.
...
-- Role grants for fops_app. The running app must never DDL pg-boss schema —
-- it only writes jobs, reads queues, and runs the create_queue/delete_queue
-- stored functions. Future pg-boss schema upgrades land as a new migration
-- generated via `getMigrationPlans`.
GRANT USAGE ON SCHEMA pgboss TO fops_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA pgboss TO fops_app;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA pgboss TO fops_app;

-- fops_migrate keeps full DDL rights for future pg-boss version migrations.
GRANT ALL ON SCHEMA pgboss TO fops_migrate;
GRANT ALL ON ALL TABLES IN SCHEMA pgboss TO fops_migrate;
GRANT ALL ON ALL FUNCTIONS IN SCHEMA pgboss TO fops_migrate;

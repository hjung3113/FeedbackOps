...
test.describe('/voc-clusters/$clusterId visual harness', () => {
  test('deeplinks through auth to the populated detail panel', async ({ page }) => {
    ...
    await expect(detail.getByTestId(`cluster-linked-finding-${IDS.finding}`)).toContainText(
      'FND-201',
    );
  ...
  test('adds a candidate through the Radix portal and refetches stateful detail data', async ({
    ...
    // #295 replaced the <select> picker with a list of candidate buttons so
    // existing members can stay visible but unselectable.
    await expect(dialog.getByTestId('add-voc-candidates')).toBeVisible();
    await expectVisual(page, dialog, 'voc-cluster-add-voc-dialog.png');

    await dialog.getByTestId(`add-voc-candidate-${IDS.candidateVoc}`).click();
    ...
    await expect(page.getByTestId(`cluster-member-row-${IDS.candidateVoc}`)).toContainText(
      'VOC-102',
    );
  });
  ...
  test('links an existing Finding through the picker and refetches the execution section', async ({
    ...
    expect(JSON.parse((await post).postData() ?? '{}')).toEqual(linkExistingFindingRequest);
    await expect.poll(() => mock.postedBodies).toEqual([linkExistingFindingRequest]);
    await expect(page.getByTestId(`cluster-linked-finding-${IDS.finding}`)).toContainText(
      'FND-201',
    );
  });

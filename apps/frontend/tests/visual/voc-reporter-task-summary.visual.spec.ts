...
test.describe('VOC reporter Task summary visual harness (#179)', () => {
  test('shows only the linked Task reporter summary', async ({ page }) => {
    ...
    await page.goto(`/vocs?view=inbox&selected=${VOC_REPORTER_TASK_SUMMARY_IDS.voc}`);
    ...
    await expect(page.getByText('연결된 실행 없음')).not.toBeVisible();
    await expectVisual(page, page.getByTestId('voc-detail-panel'), 'voc-reporter-task-summary.png');
  });
});

...
vi.mock('@fops/ui', () => ({
  ...
  ObjectRow: ({
    ...
    <div role="button" data-selected={selected ? 'true' : 'false'} onClick={onClick}>
      <span>{id}</span>
      <span>{title}</span>
      <span>{badges}</span>
      <span>{meta}</span>
...
vi.mock('@/features/voc-cluster/hooks/useCreateVocCluster', () => ({
  useCreateVocCluster: () => ({ mutate: vi.fn(), reset: vi.fn(), isPending: false }),
}));

vi.mock('@/features/voc-cluster/hooks/useAddClusterMember', () => ({
  useAddClusterMember: () => ({ mutate: vi.fn(), reset: vi.fn(), isPending: false }),
}));
...
vi.mock('@/features/voc-cluster/hooks/useCreateFindingFromCluster', () => ({
  useCreateFindingFromCluster: () => ({ mutate: vi.fn(), reset: vi.fn(), isPending: false }),
}));
...
describe('VOC cluster route shells', () => {
  ...
  it('renders the detail route inside the same ListShell detail framing', async () => {

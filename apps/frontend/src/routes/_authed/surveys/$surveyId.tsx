...
export function SurveyDetailRoute() {
  ...
  if (search.builder) { if (!gate.canManage) return <div className="p-6"><PermissionBlockedPanel state="blocked_not_requestable" category="Survey Builder" reason="설문 관리 권한이 없습니다." /></div>; return <SurveyBuilder survey={query.data} canManage gateState={gate.gateState} onBack={() => void navigate({ to: '/surveys/$surveyId', params: { surveyId } })} />; }

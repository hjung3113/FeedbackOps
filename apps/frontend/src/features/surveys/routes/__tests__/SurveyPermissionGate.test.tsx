...
const { useMe, usePermissionCheck } = vi.hoisted(() => ({ useMe: vi.fn(), usePermissionCheck: vi.fn() }));

vi.mock('@/lib/auth/useMe', () => ({ useMe }));
vi.mock('@/features/admin/permissions/use-permission-check', () => ({ usePermissionCheck }));

import { useSurveyManageGate } from '../SurveyPermissionGate';

const readyMe = { isLoading: false, isPending: false, isError: false };
const approved = { isPending: false, isError: false, data: { state: 'approved' } };
...
describe('useSurveyManageGate', () => {
  ...
  ] as const)('fails closed when /me is %s or survey.manage is unavailable', (state, me, permission) => {
  ...
  it('permits a cached ready /me only with an approved survey.manage decision', () => {
    ...
    expect(usePermissionCheck).toHaveBeenCalledWith({ capability: 'survey.manage', managedSystemId: 'system-1' });
  });
});

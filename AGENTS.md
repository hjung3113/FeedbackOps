...
- **FE gates.** `scripts/gates/frontend-verify-profile.json` runs `fe-typecheck-gate.mjs` (fails only newly introduced typecheck errors against its baseline) and `fe-biome-gate.mjs` (checks only branch-changed files). Biome errors fail the gate; warnings do not. Update the typecheck baseline or biome allowlist only when the existing diagnostic is intentionally accepted and documented in review.
